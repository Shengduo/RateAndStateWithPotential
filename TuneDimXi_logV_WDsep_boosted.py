...
class OptunaObj:
    ...
    def __init__(self, kwgs):
        ...
        if self.dim_xi > 0:
            ...
            XDots = torch.stack(XDots, dim=0).to(device=self.device)
            ts = torch.stack(ts, dim=0).to(device=self.device)
            fs = torch.stack(fs, dim=0).to(device=self.device)

            # Remove zero model from fs
            self.zeroXi_model.calf(Xs, XDots, ts)
            diff_fs = self.zeroXi_model.fs.detach()

            fs = fs - diff_fs

            Xs_cpu = Xs.to(device="cpu")
            XDots_cpu = XDots.to(device="cpu")
            ts_cpu = ts.to(device="cpu")
            fs_cpu = fs.to(device="cpu")
            
            # # Print debug info
            # print("=" * 40, " IN OPTUNA ", "="*40)
            # print("Xs.shape: ", Xs.shape, Xs.requires_grad)
            # print("XDots.shape: ", XDots.shape, XDots.requires_grad)
            # print("ts.shape: ", ts.shape, ts.requires_grad)
            # print("fs.shape: ", fs.shape, fs.requires_grad)

            # print("len(self.training_dataset) before: ", len(self.training_dataset))
            # print("self.training_dataset before: ", self.training_dataset)
            # print("self.training_dataset[0]: ", self.training_dataset[0])
            ...
            # self.training_dataset = torch.utils.data.Subset(self.training_dataset, range(len(self.training_dataset)))
            # print("len(self.training_dataset) after: ", len(self.training_dataset))
            # print("self.training_dataset after: ", self.training_dataset)
            # print("self.training_dataset[0]: ", self.training_dataset[0])
            del Xs, XDots, ts, fs
