...
class OptunaObj:
    ...
    def objective(self, trial):
        ...
        if res < self.bestValue:
            ...
            saveDir = './model/' + self.modelSavePrefix + '_trained_training_200_dimXi_{0}_dict'.format(self.dim_xi)
            myWD.save(saveDir)
            self.bestValue = res
...
OptKwgs = {
    ...
    'modelSavePrefix' : kwgs['prefix'] + "_GivenPoly_grid",
    'fOffSet' : 0.5109, 
    'scaling_factor' : 50., 
    'AllData' : AllData, 
    'logVFlag' : logVFlag, 
...
for dim_xi in dim_xis:
    ...
    this_study = optuna.create_study(direction='minimize', 
                                     storage="sqlite:///./jobs/{0}_training_200_dim_{1}".format(OptKwgs['modelSavePrefix'], dim_xi) + ".db", 
